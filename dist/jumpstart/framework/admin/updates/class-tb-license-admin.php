...
class Theme_Blvd_License_Admin {
	...
	function add_page() {
		if ( themeblvd_supports('admin', 'updates') && current_user_can( themeblvd_admin_module_cap( 'updates' ) ) ) {
			add_theme_page( __('Theme License', 'jumpstart'), __('Theme License', 'jumpstart'), 'manage_options', 'themeblvd-license', array( $this, 'admin_page' ) );
		}
	}
	...
	function admin_page() {
		...
		?>
		<div class="wrap">
			<h2><?php esc_html_e('Theme License', 'jumpstart'); ?></h2>
					...
					<tbody>
						<tr valign="top">
							<th scope="row" valign="top">
								<?php esc_html_e('License Key', 'jumpstart'); ?>
							</th>
							<td>
								<input id="themeblvd_license_key" name="themeblvd_license_key" type="text" class="regular-text" value="<?php echo esc_attr( $license ); ?>" />
								<label class="description" for="themeblvd_license_key"><?php esc_html_e('Enter your license key', 'jumpstart'); ?></label>
							</td>
						</tr>
						<?php if ( $license ) : ?>
							<tr valign="top">
								<th scope="row" valign="top">
									<?php esc_html_e( 'Activate License', 'jumpstart' ); ?>
								</th>
								<td>
