...
jQuery( document ).ready(
	function($) {
		...
		$( '.tb-options-js .group .collapsed' ).each(
			function(){
		...
		$( '.nav-tab-wrapper a' ).click(
			function(evt) {
				...
				$clicked_group.find( '.section-code' ).each(
					function(){
		...
		$( '.postbox > .section-toggle' ).on(
			'click', function(){
				...
				if ( $postbox.hasClass( 'closed' ) ) {
					...
					$postbox.find( '.section-code' ).each(
						function(){
		...
		function unhideHidden(){
			...
			if ( el.attr( 'checked' ) ) {
				...
				el.parent().parent().parent().nextAll().each(
					function(){
		...
		$( '.tb-presets a' ).on(
			'click', function(){
				...
				tbc_confirm(
					themeblvd.preset, {'confirm':true}, function(r) {
						if (r) {
