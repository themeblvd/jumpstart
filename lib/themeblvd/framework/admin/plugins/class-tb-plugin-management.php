...
class Theme_Blvd_Plugin_Management {
	...
	public function show_notice() {
		...
		echo '<p>'.sprintf(esc_html__('For everything to work properly with your current version of the %s theme, you should update the following plugins.', '@@text-domain'), '<em>'.$this->theme->get('Name').'</em>').'</p>';
		echo '<ol>';
		...
		echo '</ol>';
		echo '<p class="row-actions visible"><a href="'.esc_url(admin_url('plugins.php')).'">'.esc_html__('Go to Plugins Page', '@@text-domain').'</a> | <a href="'.esc_url($url).'">'.esc_html__('Dismiss this notice', '@@text-domain').'</a> | <a href="http://www.themeblvd.com" target="_blank">'.esc_html__('Visit ThemeBlvd.com', '@@text-domain').'</a></p>';
		echo '</div>';
	}
