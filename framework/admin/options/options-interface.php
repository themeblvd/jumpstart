...
function themeblvd_option_fields( $option_name, $options, $settings, $close = true ) {
	...
	foreach ( $options as $value ) {
		...
		switch ( $value['type'] ) {
			...
			case 'text':
				$place_holder = ! empty( $value['pholder'] ) ? ' placeholder="'.$value['pholder'].'"' : '';
			...
			case 'textarea' :
				$place_holder = ! empty( $value['pholder'] ) ? ' placeholder="'.$value['pholder'].'"' : '';
				$cols = ( isset( $value['options'] ) && isset( $value['options']['cols'] ) ) ? $value['options']['cols'] : '8' ;
				$output .= '<textarea id="'.esc_textarea( $value['id'] ).'" class="of-input" name="'.stripslashes( esc_attr( $option_name.'['.$value['id'].']') ).'" cols="'.esc_attr( $cols ).'" rows="8"'.$place_holder.'>'.esc_textarea( $val ).'</textarea>';
				break;
