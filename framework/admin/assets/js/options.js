...
(function($, admin, l10n) {
  ...
  admin.options.mediaUploader.addFile = function($section) {
    ...
    newFrame.on('select', function() {
      ...
      if ('advanced' === uploadType) {
        size = newFrame.$el
          .find('.attachment-display-settings select[name="size"]')
          .val();
      ...
      if ('image' === attachment.attributes.type) {
        $section
          ...
          .empty()
          .hide()
          .append('<img src="' + imageUrl + '"><a class="remove-image"></a>')
          .slideDown('fast');
      }
      ...
      if ('advanced' === uploadType || 'background' === uploadType) {
        ...
        // Send Link back.
        if (link) {
          linkOption = newFrame.$el
            .find('.attachment-display-settings .link-to')
            .val();

          if ('none' !== linkOption) {
            linkUrl = newFrame.$el
              .find('.attachment-display-settings .link-to-custom')
              .val();

            $section
              .closest('.advanced-image-upload')
              .find('.receive-link-url input')
              .val(linkUrl);
          }
        }
      }
    ...
    if ('advanced' === uploadType) {
      ...
      newFrame.$el
        .find('.attachment-display-settings label:first-of-type')
        .remove();
    ...
    newFrame.on('select update insert', function(event) {
      ...
      $.post(ajaxurl, data, function(response) {
        ...
        $newItems.find('.delete-sortable-item').on('click', function(event) {
          ...
          admin.confirm($link.attr('title'), { confirm: true }, function(
            response
          ) {
            if (response) {
        ...
        $newItems.find('.item-handle h3').each(function() {
          ...
          if ($trigger.is('select')) {
            $handle
              ...
              .find('.item-handle h3')
              .text(
                $trigger.find('option[value="' + $trigger.val() + '"]').text()
              );
          } else {
            $handle
              .closest('.item')
    ...
    $(element)
      ...
      .each(function() {
        ...
        if ('mini' != style && themeblvdL10n.add_shortcode) {
          $shortcodeButton = $(
            '<a href="#" class="tb-insert-shortcode button" title="' +
              themeblvdL10n.add_shortcode +
          ...
          $shortcodeButton.on('click', function(event) {
            ...
            $('body').addClass(
              'themeblvd-stop-scroll themeblvd-shortcode-generator-on'
            );
          });
        }
      });
...
(function($, admin, l10n) {
  ...
  admin.options.columnWidths.build = function($section) {
    ...
    $section
      .find('.select-col-num')
      .on('change.ui-slider', admin.options.columnWidths.change);

    $section.find('.select-grid-system').off('change.ui-slider'); // Avoid duplicates.

    $section
      .find('.select-grid-system')
      .on('change.ui-slider', admin.options.columnWidths.change);
    ...
    if (0 == columns) {
      ...
      $section
        .find('.slider')
        .append(
          '<div class="column-preview col-1" style="width:100%"><span class="text">100%</span></div>'
        );

      $section
        .find('.column-width-input')
        .val('1/1')
        .trigger('themeblvd-update-columns');
    ...
    $('#' + id).slider({
      ...
      slide: function(event, ui) {
        ...
        prevColFraction = admin.options.columnWidths.reduceFraction(
          prevNumerator,
          grid
        );

        prevColFraction =
          prevColFraction[0].toString() + '/' + prevColFraction[1].toString();

        // Reduce next column fraction
        nextColFraction = admin.options.columnWidths.reduceFraction(
          nextNumerator,
          grid
        );

        nextColFraction =
          nextColFraction[0].toString() + '/' + nextColFraction[1].toString();

        // Set hidden fraction placeholders for reference.
        $container
          .find('input[name="col[' + prevCol + ']"]')
          .val(prevColFraction);

        $container
          .find('input[name="col[' + nextCol + ']"]')
          .val(nextColFraction);
    ...
    $element.find('.tb-sortable-option').each(function() {
      ...
      $section.find('.add-item').on('click', function(event) {
        ...
        $.post(ajaxurl, data, function(response) {
          ...
          if (
            max > 0 &&
            $option.find('.item-container > .item').length >= max
          ) {
    ...
    $item.find('.item-handle h3').each(function() {
      ...
      if ($trigger.is('select')) {
        $handle
          .closest('.item')
          .find('.item-handle h3')
          .text($trigger.find('option[value="' + $trigger.val() + '"]').text());
      } else {
        $handle
          .closest('.item')
          .find('.item-handle h3')
