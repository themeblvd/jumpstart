...
(function($, admin, l10n) {
  ...
  admin.modal.init = function(element, settings) {
    ...
    self.$element.on('click.themeblvd-modal', function(event) {
      ...
      // ID for modal.
      self.id = self.settings.vimeo
        ? 'video-' + self.settings.vimeo
        : self.target;
      ...
      if (self.settings.padding) {
        self.$modalWindow
          .find('.media-frame-content-inner')
          .css('padding', '20px');
      }
      ...
      self.$modalWindow
        ...
        .off('click.themeblvd-modal-close');

      self.$modalWindow
        .find('.media-modal-close')
        .on('click.themeblvd-modal-close', function(event) {
          event.preventDefault();
      ...
      // Save.
      self.$modalWindow
        .find('.media-button-insert')
        .off('click.themeblvd-modal-insert');

      self.$modalWindow
        .find('.media-button-insert')
        .on('click.themeblvd-modal-insert', function(event) {
          event.preventDefault();
      ...
      if (self.settings.buttonSecondary) {
        self.$modalWindow
          .find('.media-button-secondary')
          .off('click.themeblvd-modal-secondary');

        self.$modalWindow
          .find('.media-button-secondary')
          .on('click.themeblvd-modal-secondary', function(event) {
            event.preventDefault();
      ...
      if (self.settings.buttonDelete) {
        var deleteMsg = self.$element.parent().is('.block-nav')
          ? l10n.delete_block
          : l10n.delete_item;

        self.$modalWindow
          .find('.media-button-delete')
          .off('click.themeblvd-modal-delete');

        self.$modalWindow
          .find('.media-button-delete')
          .on('click.themeblvd-modal-delete', function(event) {
            event.preventDefault();
  ...
  admin.modal.build = function(self) {
    self.popup =
      '<div id="%id%" class="themeblvd-modal-wrap build" style="display:none;"> \
    ...
    if (self.settings.form) {
      self.popup = self.popup.replace(
        '%content%',
        '<div class="tb-options-wrap"></div>'
      );
    }

    if (self.settings.vimeo) {
      self.popup = self.popup.replace(
        self.settings.height + '-height-modal',
        'modal-video'
      );
      self.popup = self.popup.replace(
        '%content%',
    ...
    if (
      ...
      !self.settings.buttonSecondary &&
      !self.settings.buttonDelete
    ) {
    ...
    if (self.settings.buttonSecondary) {
      $current
        .find('.media-toolbar')
        .prepend(
          '<div class="media-toolbar-secondary"><a href="#" class="button media-button button-secondary button-large media-button-secondary">' +
            self.settings.buttonSecondary +
            '</a></div>'
        );
    }
    ...
    if (self.settings.buttonDelete) {
      $current
        .find('.media-toolbar')
        .prepend(
          '<div class="media-toolbar-secondary"><a href="#" class="button media-button button-secondary button-large media-button-delete">' +
            self.settings.buttonDelete +
            '</a></div>'
        );
    }
  ...
  admin.modal.display = function(self) {
    ...
    if (self.secondary) {
      ...
      self.$modalWindow
        .find('.themeblvd-modal')
        .removeClass('themeblvd-first-modal')
        .addClass('themeblvd-second-modal');

      $body
        .find('.themeblvd-first-modal')
        .prepend('<div class="inline-media-modal-backdrop"></div>');
    } else {
      ...
      $body.append(
        '<div id="themeblvd-modal-backdrop" class="media-modal-backdrop"></div>'
      );

      // Add class to denote original base modal.
      self.$modalWindow
        .find('.themeblvd-modal')
        .addClass('themeblvd-first-modal');

      // Close all open modals.
      $('#themeblvd-modal-backdrop').off('click.themeblvd-modal-backdrop');

      $('#themeblvd-modal-backdrop').on(
        'click.themeblvd-modal-backdrop',
        function(event) {
          event.preventDefault();
  ...
  admin.modal.closeAll = function(self) {
    $('.themeblvd-modal-wrap').each(function() {
      ...
      $('body').removeClass(
        'themeblvd-modal-on themeblvd-secondary-modal-on themeblvd-stop-scroll'
      );
    });
  };
